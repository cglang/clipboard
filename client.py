...
class Client(TcpSocket):
    def __init__(self, host, port, bytehandle):
        socketconn = socket.socket()
        socketconn.connect((host, port))
        self.client_id = str(uuid.uuid4())
        super().__init__(socketconn, bytehandle)
...
#                     pyperclip.copy(text)
#                     print(
#                         '[+] {0} Clipboard synced from {1}: {2}'.format(self.get_time(), sender, text))
...
#                     print(
#                         '[+] {0} Clipboard change detected: {1}'.format(self.get_time(), self.recent_text))
#                     if send_type == 'public':
#                         self.send_public()
